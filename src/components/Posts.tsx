...
export const Posts: React.FC<Posts> = ({ posts }) => (
  <div className="grid grid-cols-1 px-6 xl:px-0 md:grid-cols-2 xl:grid-cols-3 gap-12 max-w-7xl mx-auto py-16">
    {posts.map(({ date, slug, title, description, image, duration }) => (
      <article
        className="transition transform ease-in-out delay-75 md:hover:-trangray-y-2 md:hover:scale-110 rounded-lg flex justify-center"
        key={slug}
      >
            ...
            <a className="no-underline">
              <article className="">
                ...
                <div className="mt-4">
                  ...
                  <div className="p-4">
                    <h2 className="text-3xl font-display font-semibold mb-4 text-gray-800 dark:text-gray-50">
                      {title}
                    </h2>
                    <p className="text-lg font-body text-gray-800 dark:text-gray-50">{description}</p>
                  </div>
                </div>
              </article>
            </a>
